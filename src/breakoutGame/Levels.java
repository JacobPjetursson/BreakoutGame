...
import breakoutGame.Game.STATE;
...
public class Levels {
	...
	public void tick() {
		...
		if (hud.getScore() == scoreKeep * 100) {
			...
			for (int i=0; i<handler.object.size();i++) {
				...
				if (tempObject.getId() == ID.Ball) {
					if (tempObject.getVelY()!=0) {
						Game.gameState = STATE.Nextlevel;
			...
			if (hud.getLevel() == 1 && Game.gameState==STATE.Game) {
				for (int i = 0; i < 13; i++) {
				...
				for (int i = 0; i < 13; i++) {
					...
					/* COLLISION DEBUGGING
				for (int i =0; i<=10;i++) {
					handler.addObject(new ToughEnemy(100+i*100, 100, ID.ToughEnemy, handler, hud, Color.MAGENTA));
					scoreKeep+=2;
				}
				for (int i =0; i<=10;i++) {
					handler.addObject(new ToughEnemy(100+i*100, 200, ID.ToughEnemy, handler, hud, Color.YELLOW));
					scoreKeep+=2;
				}
				for (int i =0; i<=10;i++) {
					handler.addObject(new ToughEnemy(100+i*100, 300, ID.ToughEnemy, handler, hud, Color.blue));
					scoreKeep+=2;
				}
				for (int i =0; i<=10;i++) {
					handler.addObject(new ToughEnemy(100+i*100, 400, ID.ToughEnemy, handler,hud, Color.green));
					scoreKeep+=2;
				}
					 */
			...
			else if (hud.getLevel() == 2 && Game.gameState==STATE.Game) {
				for (int i=0;i<5;i++) {
					for (int j=0;j<13;j++) {
			...
			else if (hud.getLevel() == 3 && Game.gameState==STATE.Game) {
				for (int i=0;i<7;i++) {
					for (int j=0;j<13;j++) {
						if (i%2==0) {
			...
			else if (hud.getLevel() == 4 && Game.gameState==STATE.Game) {
				for (int i=0;i<10;i++) {
					for (int j=0;j<13;j++) {
			...
			else if (hud.getLevel() == 5 && Game.gameState==STATE.Game) {
				for (int i=0;i<10;i++) {
					for (int j=0;j<13;j++) {
						if ((i<=6 && i>=3 && j<=7 && j>=5)) {
							handler.addObject(new ImmuneEnemy(80+j*enemySpaceX, 50+i*enemySpaceY, ID.ImmuneEnemy, handler, hud));
			...
			else if (hud.getLevel() == 6 && Game.gameState==STATE.Game) {
				for (int i = 0; i < 16; i++) {
					for (int j=0;j<13;j++) {
						if (j % 3 == 0) {
						handler.addObject(new BasicEnemy(80+j*enemySpaceX, 50+i*enemySpaceY, ID.BasicEnemy, handler, hud));
				...
				for (int i =0; i<10; i++) {
					handler.addObject(new ImmuneEnemy(0+i*1120, 600, ID.ImmuneEnemy, handler, hud));
				}
				
			}
			else if (hud.getLevel() == 7 && Game.gameState==STATE.Game) {
				for (int i=0;i<16;i++) {
					for (int j=0;j<13;j++) {
						handler.addObject(new ToughEnemy(80+j*enemySpaceX, 50+i*enemySpaceY, ID.ToughEnemy, handler, hud, true));
						scoreKeep+=3;
